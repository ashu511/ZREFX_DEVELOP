...
CLASS lhc_workflowinfo IMPLEMENTATION.
  ...
  METHOD updateParentStatus.
    ...
    READ ENTITIES OF zrefx_i_complaints IN LOCAL MODE
      ENTITY WorkflowInfo
      FIELDS ( ComplaintId CurrentStatus ) WITH CORRESPONDING #( keys )
      RESULT DATA(lt_wf_logs).

    DATA lt_parent_update TYPE TABLE FOR UPDATE zrefx_i_complaints.
...
*      DATA(lv_new_parent_status) = '02'.
      ...
      IF lv_new_parent_status IS NOT INITIAL.
        APPEND VALUE #( %tky   = VALUE #( ComplaintId = ls_wf_log-ComplaintId )
    ...
    IF lt_parent_update IS NOT INITIAL.
      SORT lt_parent_update BY %tky-ComplaintId.
      DELETE ADJACENT DUPLICATES FROM lt_parent_update COMPARING %tky-ComplaintId.
    ENDIF.
...
*      IMPORTING keys FOR ACTION Attachments~upload RESULT result.
...
*
*    " Result type based on your BDEF definition
*    DATA: lt_action_result TYPE TABLE FOR ACTION RESULT ZREFX_I_ATT_COMPlAINTS~upload.
*
*    " 1. Initialize CMIS Client & Repo
*    SELECT SINGLE config_value FROM zrefx_btp_config
*      WHERE object_id = 'repository_id'
*      INTO @lv_repo_id.
*
*    IF lv_repo_id IS INITIAL.
*      " Append error to REPORTED if config is missing
*      RETURN.
*    ENDIF.
*
...
*
*      DATA(lv_complaintid) = <ls_key>-%param-ComplaintId. " Assuming this is in your ZREFX_A_DMS_UPD_COM_IN struct
*      DATA(lv_folder_path) = |/{ lv_complaintid }|.
...
*                          values = VALUE #( ( string_value = lv_complaintid ) ) ) TO lt_properties.
*
...
*          APPEND VALUE #( id = cl_cmis_property_ids=>name
*                          values = VALUE #( ( string_value = <ls_key>-%param-Filename ) ) ) TO lt_properties.
*
*          ls_content-filename  = <ls_key>-%param-Filename.
*          ls_content-mime_type = <ls_key>-%param-Mimetype.
*          ls_content-stream    = <ls_key>-%param-Content.
*
...
*
*          DATA(lv_dmsid) = VALUE string( ls_new_doc-properties-properties[ id = cl_cmis_property_ids=>object_id ]-value[ 1 ]-string_value OPTIONAL ).
*
*          " DB Update Logic (RAP Buffer)
*          " Create the attachment record in SAP linked to the Complaint
*          IF lv_dmsid IS NOT INITIAL.
*
*            TRY.
*                DATA(lv_new_attachment_id) = cl_system_uuid=>create_uuid_c32_static( ).
*              CATCH cx_uuid_error.
*                CONTINUE.
*            ENDTRY.
*
...
*                FIELDS ( AttachmentId Dmsid Filename Mimetype Documentname )
*                WITH VALUE #( ( ComplaintId = lv_complaintid
*                                %target = VALUE #( ( %cid         = 'NEW_ATT'
*                                                     AttachmentId = lv_new_attachment_id
*                                                     Dmsid        = lv_dmsid
*                                                     Filename     = <ls_key>-%param-Filename
*                                                     Mimetype     = <ls_key>-%param-Mimetype
*                                                     Documentname = <ls_key>-%param-Filename ) ) ) )
*              FAILED DATA(lt_failed)
*              REPORTED DATA(lt_reported).
*
*            " Populate your result structure for the UI
*            APPEND VALUE #( %cid   = <ls_key>-%cid
*                            %param = VALUE #( AttachmentId = lv_new_attachment_id
*                                              documentid        = lv_dmsid ) " Assuming these match ZREFX_A_DMS_UPD_COM_OUT
*                          ) TO lt_action_result.
*          ENDIF.
*
*        CATCH cx_cmis_root.
*          " IMPORTANT: Pass the error back to the UI5 application
*          APPEND VALUE #( %cid = <ls_key>-%cid ) TO failed-attachments.
*          APPEND VALUE #( %cid = <ls_key>-%cid
*                          %msg = new_message_with_text( text = 'DMS Upload Failed' severity = if_abap_behv_message=>severity-error )
...
*  ENDMETHOD.
  ...
  METHOD delete.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<ls_key>).
      ...
      DATA(lv_complaintid) = <ls_key>-%param-complaintid.
      ...
      READ ENTITIES OF zrefx_i_complaints IN LOCAL MODE
        ENTITY Attachments
        FIELDS ( Dmsid )
        WITH VALUE #( ( ComplaintId = lv_complaintid AttachmentId = lv_attachmentid ) )
        RESULT DATA(lt_attachments).
      ...
      MODIFY ENTITIES OF zrefx_i_complaints IN LOCAL MODE
        ENTITY Attachments
          DELETE FROM VALUE #( ( ComplaintId = lv_complaintid AttachmentId = lv_attachmentid ) ).
  ...
  METHOD updateDmsId.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<ls_key>).
      ...
      MODIFY ENTITIES OF zrefx_i_complaints IN LOCAL MODE
        ...
        WITH VALUE #( ( ComplaintId  = <ls_key>-ComplaintId
...
* Workflow Information
  ...
  METHOD save_modified.
    ...
    DATA lt_submitted_complaints TYPE TABLE OF zrefx_complaint-complaint_id.
    ...
    IF create-complaints IS NOT INITIAL.
      ...
      LOOP AT create-complaints INTO DATA(ls_complaint).
        ...
        IF ls_complaint-Status = '02'.

          APPEND ls_complaint-ComplaintId TO lt_submitted_complaints.
          ...
          TRY.
              ...
              lo_operation->set_context_data(
                              ...
                              submittedby          = CONV string( ls_complaint-requestoremail ) "lv_user_name
    ...
    IF update-complaints IS NOT INITIAL.
      LOOP AT update-complaints INTO DATA(ls_update).
        ...
        IF ls_update-%control-Status = if_abap_behv=>mk-on AND ls_update-Status = '02'.
          ...
          APPEND ls_update-ComplaintId TO lt_submitted_complaints.

          " We must read the full active record from the DB/Buffer to send to SBPA
          READ ENTITIES OF zrefx_i_complaints IN LOCAL MODE
            ENTITY Complaints ALL FIELDS WITH VALUE #( ( ComplaintId = ls_update-ComplaintId ) )
            RESULT DATA(lt_full_complaint).

          IF lt_full_complaint IS NOT INITIAL.
            DATA(ls_full) = lt_full_complaint[ 1 ].
            TRY.
                lo_operation = NEW zcl_refx_bgpf_complaints_sbpa( ).
                lv_user_name = cl_abap_context_info=>get_user_formatted_name( ).
                lo_operation->set_context_data(
                  complaintid          = ls_full-ComplaintId
                  categorycode         = ls_full-Complaintcategory
                  ...
                  complainant          = ls_full-Vendorname
                  requestorowner       = ''  " Map to actual field if available
                  maindvmorgunit       = ls_full-MainDivision
                  isgovernmentinvolved = abap_false
                  requireslegal        = abap_false
                  ...
                  complaintype         = ls_full-Complainttype
                ).
                lo_process = cl_bgmc_process_factory=>get_default( )->create( ).
    ...
    IF lt_submitted_complaints IS NOT INITIAL.

      READ ENTITIES OF zrefx_i_complaints IN LOCAL MODE
        ENTITY Complaints BY \_Attachments
        FIELDS ( ComplaintId AttachmentId Filename Mimetype Content )
        WITH VALUE #( FOR comp IN lt_submitted_complaints ( ComplaintId = comp ) )
      ...
      LOOP AT lt_attachments_to_upload INTO DATA(ls_att).
        ...
        IF ls_att-Content IS NOT INITIAL.
          APPEND VALUE #( complaintid  = ls_att-ComplaintId
...
*  ENDMETHOD.
  ...
  METHOD UpdateDivision.
    ...
    DATA: lt_action_result TYPE TABLE FOR ACTION RESULT zrefx_i_complaints~UpdateDivision.
    ...
    IF sy-subrc = 0.
      ...
      MODIFY ENTITIES OF zrefx_i_complaints IN LOCAL MODE
        ENTITY Complaints
          UPDATE FIELDS ( MainDivision MainDivDescription Legalflag )
*          WITH VALUE #( ( %tky         = <fs_key>-%tky
          WITH VALUE #( ( ComplaintId        = <fs_key>-%param-ComplaintId
                          MainDivision       = <fs_key>-%param-MainDivision
                          MainDivDescription = <fs_key>-%param-MainDivDescription
...
*        RESULT DATA(lt_complaints).
      READ ENTITIES OF zrefx_i_complaints IN LOCAL MODE
              ENTITY Complaints
                ALL FIELDS WITH VALUE #( ( ComplaintId = <fs_key>-%param-ComplaintId ) )
