CLASS zcl_refx_bgpf_compns_dms_up DEFINITION
   PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_refx_bgpf_compns_dms_up IMPLEMENTATION.
...
* Group by Complaint ID to avoid redundant folder checks/creation
    ...
    DATA(lv_folder_path) = |/{ ls_first_item-compensationid }|.
...
* Create folder if missing
        ...
        APPEND VALUE #( id = cl_cmis_property_ids=>name
                        values = VALUE #( ( string_value = ls_first_item-compensationid ) ) ) TO lt_properties.
...
*            ENDIF.
            ...
            MODIFY ENTITIES OF zrefx_i_compensation
                          ENTITY Attachments
                            EXECUTE updateDmsId
                            FROM VALUE #( ( CompensationId  = <ls_ctx>-compensationid
