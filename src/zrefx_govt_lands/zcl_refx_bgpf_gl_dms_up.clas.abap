CLASS zcl_refx_bgpf_gl_dms_up DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_refx_bgpf_gl_dms_up IMPLEMENTATION.
  ...
  METHOD if_bgmc_op_single~execute.
    ...
    " Using your BTP config table (matching the Complaints app pattern)
    ...
    " Group by Request ID to avoid redundant folder checks/creation
    DATA(ls_first_item) = mt_context_data[ 1 ].
    DATA(lv_folder_path) = |/{ ls_first_item-requestid }|.
    ...
    TRY.
      ...
      CATCH cx_cmis_object_not_found.
        ...
        APPEND VALUE #( id = cl_cmis_property_ids=>name
                        values = VALUE #( ( string_value = ls_first_item-requestid ) ) ) TO lt_properties.
    ...
    LOOP AT mt_context_data ASSIGNING FIELD-SYMBOL(<ls_ctx>).
      TRY.
          ...
          IF <ls_ctx>-documentid IS NOT INITIAL.
            ...
            " Use the exact Action pattern from Complaints to bypass UI Read-Only locks
            MODIFY ENTITIES OF zrefx_i_gl_request
              ENTITY ReqAttachment
                EXECUTE updateDmsId
                FROM VALUE #( ( RequestId    = <ls_ctx>-requestid
            ...
            IF lt_failed IS INITIAL.
              " bgPF handles the COMMIT automatically
            ENDIF.
