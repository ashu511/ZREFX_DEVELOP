...
define  view entity ZREFX_I_GL_SITEVISIT as select from zrefx_row_sitevi

  association        to parent ZREFX_I_GL_PROPOSEDLAND     as _ProposedLand     on  $projection.RequestId  = _ProposedLand.RequestId
  ...
  association [0..*] to ZREFX_I_GL_SITEVISITNOM        as _SiteVisitNom on  $projection.RequestId = _SiteVisitNom.RequestId


 // association [0..*] to ZREFX_I_USERREF             as _UserRef      on  $projection.Engineer = _UserRef.UserID

  association        to ZREFX_I_GL_STATUS              as _Status       on  _Status.Statustype     = 'SITEVISIT'
                                                                     and $projection.Statuscode = _Status.Code

  association [1..1] to ZREFX_I_GL_WORKFLOW_INSTANCE   as _Workflow     on  _Workflow.Objecttype = 'SITEVISIT'

  composition of many ZREFX_I_GL_SITEVISIT_ATT as _Attachments
{
